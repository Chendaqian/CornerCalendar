...
**[English](README.md) | 简体中文| [WIKI](https://github.com/Chendaqian/CornerCalendar/wiki)**
...
CornerCalendar 是一款 Windows 日历小工具，用紧凑的月历面板替换任务栏日历弹窗。程序常驻系统托盘，托盘图标是逐帧动画"跑者"，帧速随 CPU 负载动态变化，点击跑者即可打开日历主面板，不需要启动完整的桌面日历应用。
...
- 托盘跑者动画参考 [RunCat365](https://github.com/runcat-dev/RunCat365)，内置 40 个来自 [RunnerGallery](https://runcat-dev.github.io/RunnerGallery/) 的跑者：帧速随 CPU 负载动态变化，深色主题自动反白，设置窗口可预览并选择跑者。
- 支持托盘右键菜单、开机自启动、浅色/深色/跟随系统主题、字号设置、设置/关于/更新页面。
- 不覆盖或修改任何显示器的任务栏，Windows 原生任务栏时间和通知中心保持原样。
...
设置窗口可以配置主题、字号、开机自启动、用户 ICS 订阅、ICS 刷新频率、天气位置、天气服务地址、天气刷新频率、显示选项和托盘跑者。天气刷新频率默认 120 分钟，也可以设置为 30、60 或 240 分钟。
...
└── CornerCalendar/
    ...
    ├── Core/Models/                   # 日历、中国日历、天气和跑者模型
    ├── Core/Services/                 # 设置、ICS、中国日历和天气服务
    ├── Core/Helpers/                  # Win32、主题、农历、自启动、跑者动画和图标帮助类
