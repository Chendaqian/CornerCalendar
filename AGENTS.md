...
CornerCalendar 是 Windows 10/11 上的任务栏日历工具：应用常驻系统托盘，托盘图标是逐帧动画"跑者"（帧速随 CPU 负载动态变化，参考 RunCat365），点击跑者后从底部弹出月历面板；面板提供农历、中国大陆法定节假日和调休、二十四节气、ICS 日程、天气摘要以及日期详情窗口。
...
| Win32 / WPF interop | 定位主显示器任务栏、弹出窗口定位 |
| RunCat365（Apache-2.0）/ RunnerGallery | 托盘跑者动画参考实现与跑者帧资源来源 |
...
└── CornerCalendar/
    ├── App.xaml(.cs)                     # 组合根、托盘跑者动画和生命周期
    ├── Core/
    │   ├── Models/                       # CalendarDay、CalendarEvent、天气、中国日历和跑者模型
    │   ├── Services/                     # 设置、ICS、中国日历、天气和聚合服务
    │   └── Helpers/                      # Win32、主题、农历、自启动、日志、跑者动画和图标帮助类
    ├── ViewModels/                       # CalendarViewModel、EventListViewModel
    └── Views/
        ├── PopupWindow.xaml(.cs)         # 主月历面板、天气切换和动画
        ├── SettingsWindow.xaml(.cs)      # 分类设置窗口，含跑者预览
...
交付前至少确认：构建 0 错误 0 警告、测试通过；涉及窗口行为时还要在 Windows 上手工检查托盘跑者动画、主面板、设置窗口，以及各显示器系统原生任务栏时钟和控制中心不受影响。
...
1. `App.xaml.cs` 是组合根，负责托盘图标与跑者动画器（`TrayRunnerAnimator`）、天气后台刷新、弹窗/设置窗口生命周期和全局异常日志。
...
7. 托盘跑者动画帧速随 CPU 负载动态变化，跑者资源位于 `Resources/Runners`（每个文件夹一个跑者）；不得在任何显示器上创建任务栏时间覆盖窗口，必须保留系统原生任务栏时间和通知中心。
...
- 退出时必须停止跑者动画与 CPU 采样、释放帧图标缓存并移除托盘图标；系统任务栏在所有显示器上保持原生行为。
