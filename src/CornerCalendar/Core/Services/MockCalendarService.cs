...
public class MockCalendarService : ICalendarService
{
    ...
    public Task<List<CalendarEvent>> GetEventsAsync(DateTime start, DateTime end)
    {
        ...
        for (int i = 0; i < 14; i++)
        {
            ...
            int eventCount = rng.Next(0, 4); // 每天 0~3 个事件

            for (int j = 0; j < eventCount; j++)
            {
                int hour = rng.Next(8, 18);
                int minute = rng.Next(0, 4) * 15; // 0, 15, 30, 45
                int durationHours = rng.Next(1, 4);
                bool isAllDay = rng.Next(10) < 2; // 20% 概率全天事件
    ...
    {
        Debug.WriteLine("CornerCalendar: Mock - would open system calendar app.");
    }
    ...
    {
        string[] calendars = new[] { "Outlook", "Google Calendar", "iCloud" };
